👩‍💻 Nome: Michele Francisca de Almeida

🎓 Formação: Bacharelado em Ciência da Computação - PUC Minas  
💻 Foco: Desenvolvimento, lógica de programação e algoritmos  
📍 Localização: Brasil  

---

## 🚀 Tecnologias e Ferramentas
C • C++ • Java • JavaScript • HTML • HTML5 • CSS • CSS3 • Git • GitHub  

---

## 🧩 Habilidades (AEDs 1)
C • C++ • Condicional • Entrada/Saída • Funções • Loops • Matemática • Matrizes • Modularização • Ponteiros • Structs • Switch • Variáveis • Vetores  

---

## 📂 Exercícios Destacados (AEDs 1)

| Exercício | Descrição | Conceitos |
|----------|----------|----------|
| Ex 1 | Área e perímetro do retângulo | if, else, operações matemáticas |
| Ex 2 | Área do trapézio | if, else, cálculo de área |
| Ex 3 | Áreas | if, else, operações matemáticas |
| Ex 4 | Retângulo: área, perímetro e diagonal | if, else, fórmulas |
| Ex 5 | Salário e horas trabalhadas | if, else, aritmética |
| Ex 6 | Consumo de combustível | if, else, média |
| Ex 7 | Positivo, negativo ou zero | if, else |
| Ex 8 | Maior de dois números | if, else |
| Ex 9 | Par ou ímpar | if, else |
| Ex 10 | Média com situação | if, else |
| Ex 11 | Maior de três números | if, else |
| Ex 12 | Múltiplos | if, else |
| Ex 13 | Intervalo | if, else |
| Ex 14 | Intervalos múltiplos | if, else |
| Ex 15 | Lanche (clássico) | if, else |
| Ex 16 | Aumento de salário | if, else |
| Ex 17 | Tipo de triângulo | if, else |

---

## 📂 Outros Projetos

### 🔹 Projetos de LIP — 1º Período (2025)
Projetos realizados na disciplina de Laboratório de Iniciação à Programação (LIP).  

**Conceitos aplicados:**  
- Entrada e saída de dados  
- Estruturas condicionais  
- Loops (`for`, `while`)  
- Operações básicas  
- Lógica de programação  

---

### 🔹 Calculadora em C
Aplicação desenvolvida em linguagem C com foco em programação estruturada e lógica de programação.  

🧠 **Habilidades demonstradas:**  
- Lógica de programação  
- Estruturas condicionais (`if`, `switch`)  
- Estruturas de repetição (`while`)  
- Funções  
- Entrada e saída de dados  

📊 Nível: Básico → Intermediário  
🎯 Objetivo: Consolidar fundamentos da linguagem C  

---

### 🔹 Calculadora em C++
Aplicação desenvolvida em C++ com foco na evolução da lógica de programação.  

🧠 **Habilidades demonstradas:**  
- Adaptação de lógica de C para C++  
- Estruturas condicionais (`if`, `switch`)  
- Estruturas de repetição (`while`)  
- Funções  
- Entrada e saída com `cin` e `cout`  

📊 Nível: Básico → Intermediário  
🎯 Objetivo: Preparação para Programação Orientada a Objetos  

---

## 🌐 Projetos de DIW – Desenvolvimento de Interfaces Web

### 🔹 Semana 3
Homepage interativa com formulário  
**Tecnologias:** HTML, CSS, Bootstrap, JavaScript  

### 🔹 Semanas 4 e 5
Responsividade (mobile e desktop)  
**Tecnologias:** HTML, CSS  

### 🔹 Semanas 7 e 8
Páginas dinâmicas  
**Tecnologias:** HTML, CSS, JavaScript  

### 🔹 Semanas 11 e 12
CRUD com JSON Server  
**Tecnologias:** HTML, CSS, JavaScript  

### 🔹 Semanas 13 e 14
Visualização com gráficos e mapas  
**Tecnologias:** HTML, CSS, JavaScript  

---

## 🎯 Objetivo
Buscando oportunidade de estágio em desenvolvimento para evoluir minhas habilidades em programação e interfaces web.  

---

## 📫 Contato
GitHub: MicheleFranciscaDeAlmeida  
